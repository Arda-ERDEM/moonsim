...
def _line_of_sight(
    ...
    slope_mask = None if impassable_slope_mask is None else np.asarray(
        impassable_slope_mask, dtype=bool)
...
def theta_star_plan(
    ...
    inflated_blocked = np.asarray(inflated_blocked, dtype=bool) | np.asarray(
        impassable_slope_mask, dtype=bool)
    inflated_blocked[start] = False
    inflated_blocked[goal] = False
    ...
    while frontier and expansions < max_expansions:
        ...
        for dr, dc in neighbors_8:
            ...
            dz_neighbor = float(
                elevation_map[nr, nc] - elevation_map[curr_r, curr_c])
            ...
            if elevation_weight > 0 and elevation_map is not None:
                elev_delta = elevation_map[nr, nc] - \
                    elevation_map[curr_r, curr_c]
                step_cost = _elevate_cost(
                    step_cost, elev_delta, elevation_weight)
            ...
            if parent_node is not None:
                ...
                if _line_of_sight(
                    ...
                    shortcut_cost = distance_weight * shortcut_dist * (
                        0.5 * (cost_map[grandparent[0],
                               grandparent[1]] + cost_map[nr, nc])
                    )

                    if elevation_weight > 0:
                        elev_delta = elevation_map[nr, nc] - \
                            elevation_map[grandparent[0], grandparent[1]]
                        shortcut_cost = _elevate_cost(
                            shortcut_cost, elev_delta, elevation_weight)
                    ...
                    if neighbor not in g_cost or candidate_g < g_cost[neighbor]:
                        ...
                        priority = candidate_g + \
                            _heuristic_euclidean(neighbor, goal)
...
def _interpolate_path_3d(
    ...
    elevs = np.array([elevation_map[int(r), int(c)]
                     for r, c in grid_path], dtype=float)
